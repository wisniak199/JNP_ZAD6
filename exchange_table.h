...
// Tabela wymiany walut dla banku.
class ExchangeTable {
    private:
    ...
    public:
        ...
        double getBuyRate(Currency c) const {
            return buyRates.find(c)->second;
        }
        double getSellRate(Currency c) const {
            return sellRates.find(c)->second;
        }
        ...
        ExchangeTable& exchangeRate(Currency cur) {
            _settingCurrency = cur; return *this;
        }
        ExchangeTable& buyingRate(double rate) {
            buyRates[_settingCurrency] = rate;
            return *this;
        }
        ExchangeTable& sellingRate(double rate) {
            sellRates[_settingCurrency] = rate;
            return *this;
        }
